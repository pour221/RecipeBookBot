...
async def quick_change_collection(callback: CallbackQuery, current_user: User, session: AsyncSession):
    page = 1
    page_size = 4
    photo = FSInputFile(pics['main'])
    caption_text = 'Choose collection'
    collections, has_next_page, total_pages = await get_collection_list_page(session, current_user, page, page_size)
    await callback.message.edit_media(InputMediaPhoto(media=photo,
                                                      caption=caption_text),
                                      reply_markup=get_collection_list_kb(collections, page, has_next_page, 'set_active'))
    await callback.answer()
